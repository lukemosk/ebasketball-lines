...
def upsert_event(ev, league_id):
    # Try to get finals from the event data
    final_home, final_away = None, None
    
    # If we have 'ss' field, try to parse it
    if ev.get("ss"):
        try:
            a, b = ev["ss"].split("-")
    ...
    # For live or not_started, ensure finals are NULL
    if status in ("live", "not_started"):
        final_home, final_away = None, None
    ...
    ON CONFLICT(event_id) DO UPDATE SET
      ...
      final_home=CASE 
        WHEN excluded.final_home IS NOT NULL THEN excluded.final_home 
        WHEN excluded.status IN ('live', 'not_started') THEN NULL
        ELSE event.final_home 
      END,
      final_away=CASE 
        WHEN excluded.final_away IS NOT NULL THEN excluded.final_away 
        WHEN excluded.status IN ('live', 'not_started') THEN NULL
        ELSE event.final_away 
      END;
    """
    with engine.begin() as c:
        c.execute(text(sql), {"id":ev["id"],"lid":league_id,"time":ev["time"],"status":status,
